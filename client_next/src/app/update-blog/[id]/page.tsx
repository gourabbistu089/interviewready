...
import {
  ...
  Plus, X, Upload, Trash2, Sparkles, ArrowLeft, Loader,
} from 'lucide-react';
...
export default function UpdateBlogPage() {
  ...
  const [formData,       setFormData]       = useState<BlogFormData>({ title: '', excerpt: '', content: '', featuredImage: null, category: '', tags: [], status: 'draft', readTime: 1 });
  ...
  useEffect(() => {
    const fetchBlog = async () => {
      try {
        ...
        setFormData({ title: blog.title ?? '', excerpt: blog.excerpt ?? '', content: blog.content ?? '', featuredImage: null, category: blog.category ?? '', tags: blog.tags ?? [], status: blog.status ?? 'draft', readTime: blog.readTime ?? 1 });
        if (blog.featuredImage) setExistingImage(blog.featuredImage);
      } catch {
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => { setImagePreview(ev.target?.result as string); setExistingImage(null); };
    reader.readAsDataURL(file);
  };

  const removeImage = () => { setFormData((p) => ({ ...p, featuredImage: null })); setImagePreview(null); setExistingImage(null); };
  const addTag = () => { if (newTag.trim() && !formData.tags.includes(newTag.trim())) { setFormData((p) => ({ ...p, tags: [...p.tags, newTag.trim()] })); setNewTag(''); } };
  ...
  const validateForm = () => {
    ...
    if (!formData.featuredImage && !existingImage)   e.featuredImage = 'Featured image is required';
  ...
  const handleSubmit = async (status: string) => {
    ...
    try {
      ...
      fd.append('title', formData.title); fd.append('excerpt', formData.excerpt);
      fd.append('content', formData.content); fd.append('category', formData.category);
      fd.append('tags', JSON.stringify(formData.tags)); fd.append('status', status);
      fd.append('readTime', String(formData.readTime));
      if (formData.featuredImage) fd.append('featuredImage', formData.featuredImage);

      const res = await axios.put(`${API_URL}/blogs/${id}`, fd, {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: 'var(--bg-base)' }}>
        <div className="text-center">
          <Loader className="w-10 h-10 animate-spin mx-auto mb-3" style={{ color: 'var(--accent)' }} />
          <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>Loading blog data...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          <button onClick={() => router.back()}
            className="inline-flex items-center gap-2 text-sm mb-5 transition-opacity hover:opacity-80"
          ...
          <div className="text-center">
            ...
            <h1 className="text-2xl font-bold mb-1" style={{ color: '#ffffff', fontFamily: 'Syne, sans-serif' }}>Update Blog Post</h1>
            <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>Edit and refine your content</p>
          </div>
        </div>
          ...
          <div className="p-8 space-y-8">
            ...
            <div>
              ...
              {!previewSrc ? (
                ...
                <div className="relative group">
                  ...
                  <img src={previewSrc} alt="Preview" className="w-full h-64 object-cover rounded-xl" style={{ border: '0.5px solid var(--border)' }} />
                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity rounded-xl flex items-center justify-center gap-3">
                    <label
                      className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium cursor-pointer transition-opacity hover:opacity-90"
                      style={{ background: 'var(--accent)', color: '#080808' }}
                    >
            ...
            <div>
              ...
              <textarea value={formData.excerpt} onChange={(e) => setFormData((p) => ({ ...p, excerpt: e.target.value }))}
                rows={3} className="w-full resize-none" placeholder="Write an excerpt..." />
            </div>
            ...
            <div>
              ...
              <div className="flex gap-3 mb-4">
                <input type="text" value={newTag} onChange={(e) => setNewTag(e.target.value)}
                  ...
                  className="flex-1" placeholder="Add a tag..." />
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-6" style={{ borderTop: '0.5px solid var(--border)' }}>
              <button type="button" onClick={() => handleSubmit('draft')} disabled={isLoading}
                className="flex items-center justify-center gap-2 px-7 py-3 rounded-lg text-sm font-medium transition-opacity hover:opacity-80 disabled:opacity-50"
              ...
              <button type="button" onClick={() => handleSubmit('published')} disabled={isLoading}
                ...
                style={{ background: 'var(--accent)', color: '#080808' }}
              >
                {isLoading ? <><Loader className="w-4 h-4 animate-spin" />Updating...</> : <><Send className="w-4 h-4" />Update & Publish</>}
              </button>
            </div>
          </div>
        </div>
