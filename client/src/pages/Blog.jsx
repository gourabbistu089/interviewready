...
function Blog() {
  ...
  const [liked, setLiked] = useState(
    blog?.likes?.some((like) => like.user === currentUser?._id) || false
  );
  ...
  const handleLike = async () => {
    ...
    try {
      const res = axios.post(
        `${API_URL}/blogs/${blog._id}/toggle-like`,
        {},
        {
          headers: {
  ...
  const handleBlogDelete = async () => {
    try {
      ...
      const response = await axios.delete(`${API_URL}/blogs/${deleteBlogId}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
      console.log("Response in blog delete", response);
  ...
  const handleComment = () => {
    if (newComment.trim()) {
      ...
      try {
        const res = axios.post(
          `${API_URL}/blogs/${blog._id}/add-comment`,
          {
            content: newComment,
          },
          {
  ...
  // Add this early return to prevent rendering if no blog
  if (!blog) {
              ...
              <>
                <div className="flex gap-4">
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-2 px-8 py-4 bg-red-500 text-white rounded-xl hover:bg-red-600 transition-colors font-medium shadow-lg"
                  >
